...
class PerpusController {
  ...
  static Future<bool> saveUser(User user) async {
    try {
      ...
      bool nikExists = users.any((u) => u.nik == user.nik);

      if (emailExists || usernameExists || nikExists) {
        return false; // Email, username, atau NIK sudah digunakan
      }
  ...
  static Future<User?> login(String emailOrNik, String password) async {
    try {
      ...
      User? user = users.firstWhere(
        (u) =>
            (u.email == emailOrNik || u.nik == emailOrNik) &&
            u.password == password,
        orElse: () => User(
          namaLengkap: '',
          nik: '',
